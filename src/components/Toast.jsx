  ...
  return (
    <ToastContext.Provider value={{ show }}>
      ...
      <div style={{ position: 'fixed', bottom: '80px', left: '50%', transform: 'translateX(-50%)', zIndex: 9999, display: 'flex', flexDirection: 'column', gap: '8px', width: 'calc(100% - 2rem)', maxWidth: '440px', pointerEvents: 'none' }}>
        {toasts.map(t => (
          <div key={t.id} style={{ background: bg[t.type] || bg.info, color: '#fff', borderRadius: '0.875rem', padding: '12px 16px', fontSize: '14px', fontWeight: 500, display: 'flex', alignItems: 'center', justifyContent: 'space-between', boxShadow: '0 4px 16px rgba(0,0,0,0.2)', pointerEvents: 'auto' }}>
            <span>{t.message}</span>

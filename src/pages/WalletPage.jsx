...
export default function WalletPage() {
  ...
  async function handleLookup(e) {
    ...
    try {
      ...
      if (res.status === 'success' && res.data) {
        ...
        setError('Phone number not found. Place an order first to register a wallet.')
      }
    } catch {
  ...
  const balance = Number(customer?.wallet_thb) || 0
  ...
  return (
    <div style={{ minHeight: '100dvh', background: '#FAF6F0', paddingBottom: '80px' }}>
      ...
      <div style={{ background: '#fff', borderBottom: '1px solid #E8D5C0', padding: '14px 1rem', display: 'flex', alignItems: 'center', gap: '8px' }}>
        <Wallet size={20} color="#7C3A1E" />
        <h1 style={{ margin: 0, fontSize: '18px', fontWeight: 700, color: '#2C1A0E' }}>Prepaid Wallet</h1>
      </div>
      ...
      <div style={{ padding: '1.5rem 1rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        <form onSubmit={handleLookup} style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
          <p style={{ margin: 0, fontSize: '15px', color: '#8C6A52' }}>Enter your phone number to check your balance.</p>
        ...
        {customer && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
            ...
            <div style={{ background: '#fff', borderRadius: '1.25rem', padding: '1.5rem 1rem', boxShadow: '0 2px 12px rgba(44,26,14,0.07)', display: 'flex', flexDirection: 'column', gap: '4px', alignItems: 'center' }}>
              <span style={{ fontSize: '13px', color: '#8C6A52', fontWeight: 500 }}>Available balance</span>
              <span style={{ fontSize: '36px', fontWeight: 700, color: balance > 0 ? '#7C3A1E' : '#C0392B' }}>{formatPrice(balance)}</span>
            </div>
            <p style={{ margin: 0, fontSize: '13px', color: '#8C6A52', textAlign: 'center', lineHeight: 1.5 }}>
              Top-up is done by the seller. Contact us to add funds.
            </p>
          </div>
        )}
      </div>
